package io.github.hsn.jnastool.torrent.bencoding.types;

import io.github.hsn.jnastool.torrent.bencoding.Utils;
import lombok.Getter;

import java.nio.charset.Charset;
import java.util.Arrays;

@Getter
public class BByteString implements IBencodable
{
    private final byte[] data;

    public BByteString(byte[] data)
    {
        this.data = data;
    }

    public BByteString(String name)
    {
        this.data = name.getBytes();
    }

    ////////////////////////////////////////////////////////////////////////////
    //// GETTERS AND SETTERS ///////////////////////////////////////////////////
    ////////////////////////////////////////////////////////////////////////////

    ////////////////////////////////////////////////////////////////////////////
    //// ENCODING /////////////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////////////////////////

    public String encodedString()
    {
        return data.length + ":" + new String(data);
    }

    public byte[] encode()
    {

        byte[] lengthStringAsBytes = Utils.stringToAsciiBytes(Long.toString(data.length));
        byte[] encoded = new byte[lengthStringAsBytes.length + 1 + data.length];

        encoded[lengthStringAsBytes.length] = ':';
        // Copy the length array in first.
        System.arraycopy(lengthStringAsBytes, 0, encoded, 0, lengthStringAsBytes.length);
        // Copy in the actual data.
        for (int i = 0; i < data.length; i++)
            encoded[i + lengthStringAsBytes.length + 1] = data[i];

        return encoded;
    }

    ////////////////////////////////////////////////////////////////////////////
    //// OVERRIDDEN METHODS ////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////////////////////////
    @Override
    public String toString()
    {
        return new String(data, Charset.forName("UTF-8"));
    }

    @Override
    public boolean equals(Object o)
    {
        if (this == o) return true;
        if (o == null || getClass() != o.getClass()) return false;

        BByteString that = (BByteString) o;

        return Arrays.equals(data, that.data);
    }

    @Override
    public int hashCode()
    {
        return data != null ? Arrays.hashCode(data) : 0;
    }
}
